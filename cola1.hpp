...
template <typename T> class Cola {
	public:
		...
		bool vacia() const;
		size_t tama() const;
		size_t tamaMax() const; // Requerida por la implementaci�n
	...
	private:
		...
		inicio, // Posici�n del frente
		n_eltos; // Tama�o de la cola
};
...
template <typename T>
Cola<T>::Cola(const Cola& C) :elementos(new T[C.Lmax]),Lmax(C.Lmax),inicio(0),n_eltos(C.n_eltos)
{
